...
GraphPublisher::GraphPublisher(rclcpp::Node* n, slam3d::Graph* g) : mNode(n), mGraph(g)
{
	mPosePublisher = mNode->create_publisher<visualization_msgs::msg::Marker>("vertices", 1);
	mLabelPublisher = mNode->create_publisher<visualization_msgs::msg::MarkerArray>("labels", 1);
}
...
void GraphPublisher::addEdgeSensor(const std::string& sensor)
{
	...
	name << "edges" << sensor;
	mEdgePublisherMap[sensor] = mNode->create_publisher<visualization_msgs::msg::Marker>(name.str(), 1);
}

void GraphPublisher::publishNodes(const rclcpp::Time& stamp, const std::string& frame)
{
	...
	for(VertexObjectList::const_iterator it = vertices.begin(); it != vertices.end(); it++)
	{
		...
		labels.markers[i].type = visualization_msgs::msg::Marker::TEXT_VIEW_FACING;
